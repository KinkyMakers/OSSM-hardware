...
void OSSM::startStrokeEngine() {
    ...
    Stroker.setDepth(
        0.01f * depthPercentage * abs(measuredStrokeMm), true);
    Stroker.setStroke(
        0.01f * strokePercentage * abs(measuredStrokeMm),
        true);
    ...
    for (;;) {
        ...
        if (isChangeSignificant(lastSpeedPercentage, speedPercentage)) {
            ...
            Stroker.setSpeed(speedPercentage * 3,
                             true);  // multiply by 3 to get to sane thrusts
                                     // per minute speed
        ...
        //                     buttonPressCount);
        //
        //            // If we are coming from the pattern selection, apply the
        //            new
        //            // pattern upon switching out of it. This is to prevent
        //            sudden
        //            // jarring pattern changes while scrolling through them
        ...
        //            }
        //
        //            if (buttonPressCount > 1)  // Enter pattern-selection mode
        //            if the
        //                                       // button is pressed more than
        //                                       once
        //            {
        //                rightKnobMode = MODE_PATTERN;
        //            } else if (strokePattern ==
        //                       0)  // If the button was only pressed once and
        //                       we're
        //                           // in the basic stroke engine pattern...
        //            {
        //                // ..clamp the right knob mode so that we bypass the
        //                // "sensation" mode
        //                rightKnobMode += 1;
        ...
        //                }
        //            } else {
        //                // Otherwise allow us to select the sensation control
        ...
        //
        //            strokerPatternName = Stroker.getPatternName(
        //                strokePattern);  // Update the stroke pattern name
        //                (used by
        //                                 // the UI)
        //
        //            modeChanged = true;
