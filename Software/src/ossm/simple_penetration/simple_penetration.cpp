...
namespace simple_penetration {
    ...
    static void startSimplePenetrationTask(void *pvParameters) {
        ...
        while (isInCorrectState()) {
            ...
            bool isSpeedZero = settings.speedKnob <
            ...
            // If the speed is greater than the dead-zone, and the speed has
            // changed by more than the dead-zone, then update the stepper. This
            // must be done in the same task that the stepper is running in.
            if (isSpeedChanged) {
                lastSpeed = speed;
            ...
            if (settings.speed > Config::Advanced::commandDeadZonePercentage &&
                ...
                // This calculation assumes that at the end of every stroke you
                // have a whole positive distance, equal to maximum target
                // position.
    ...
    void startSimplePenetration() {
        ...
        xTaskCreatePinnedToCore(
            ...
            &Tasks::runSimplePenetrationTaskH, Tasks::operationTaskCore);
    }
