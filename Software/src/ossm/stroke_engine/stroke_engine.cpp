...
namespace stroke_engine {
    ...
    static void startStrokeEngineTask(void *pvParameters) {
        ...
        Stroker.setStroke(0.01f * settings.stroke * abs(measuredStrokeMm),
                          true);
        ...
        while (isInCorrectState()) {
            ...
            if (lastSetting.stroke != settings.stroke) {
                float newStroke =
                    0.01f * settings.stroke * abs(measuredStrokeMm);
            ...
            if (lastSetting.depth != settings.depth) {
                ...
                ESP_LOGD("UTILS", "change depth: %f %f", settings.depth,
                         newDepth);
                ...
                if (settings.pattern < HARDCODED_PATTERN_COUNT) {
                    switch (static_cast<StrokePatterns>(settings.pattern)) {
                        case StrokePatterns::SimpleStroke:
                            Stroker.setPattern(
                                new SimpleStroke("Simple Stroke"), false);
                            break;
                    ...
                    const char *patternId = patternCatalog[settings.pattern].id;
                    ...
                    float strokeSteps = fabsf(calibration.measuredStrokeSteps);
                    float maxSpeedHz =
    ...
    static void publishStateTask(void *pvParameters) {
        ...
        const TickType_t publishInterval =
            pdMS_TO_TICKS((int)(1000.0f / UserConfig::mqttPublishFrequencyHz));
        ...
        while (isInCorrectState()) {
            ...
            int result = esp_mqtt_client_publish(mqttClient, topic.c_str(),
                                                 payload.c_str(),
    ...
    void startStrokeEngine() {
        ...
        xTaskCreatePinnedToCore(
            publishStateTask, "publishStateTask", 5 * configMINIMAL_STACK_SIZE,
            nullptr, tskIDLE_PRIORITY + 1, nullptr, Tasks::operationTaskCore);
    }
