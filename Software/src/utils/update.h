...
static auto isUpdateAvailable = []() {
    ...
    if (WiFiClass::status() != WL_CONNECTED) {
        ESP_LOGD("UTILS", "Not connected to WiFi");
        return false;
    }
...
#ifdef VERSIONDEV
    serverNameBubble =
        "http://d2oq8yqnezqh3r.cloudfront.net/"
        "check-for-ossm-update";  // version - test
#endif

#ifdef VERSIONSTAGING
    serverNameBubble =
        "http://d2oq8yqnezqh3r.cloudfront.net/check-for-ossm-update";  // version
                                                                       // - test
#endif

    ESP_LOGD("UTILS", "Checking for updates at %s",
...
#ifdef VERSIONDEV
    url = "http://d2sy3zdr3r1gt5.cloudfront.net/firmware-alpha.bin";
#endif
