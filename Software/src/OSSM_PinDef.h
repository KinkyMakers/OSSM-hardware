...
/*
    ...
    OSSM Reference board users are unlikely to need to modify this! See
   OSSM_Config.h
*/
...
// drivers. N.b. to iHSV57 users - DIP switch #5 can be flipped to invert motor
// direction entirely
...
// Pin for the toggle for wifi control (Can be left alone if no hardware toggle
// is required)
#define WIFI_CONTROL_TOGGLE_PIN 22

#define LOCAL_CONTROLLER INPUT_PULLDOWN
#define WIFI_CONTROLLER INPUT_PULLUP
// Choose whether the default control scheme is local (e.g. OSSM remote,
// potentiometers, etc.) or through Wi-Fi If both are desired, a hardware toggle
// will need to be installed and wired to WIFI_CONTROL_TOGGLE_PIN
#define WIFI_CONTROL_DEFAULT LOCAL_CONTROLLER

/*These are configured for the OSSM Remote - which has a screen, a potentiometer
 * and an encoder which clicks*/
