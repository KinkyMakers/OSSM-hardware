    ...
    uint16_t CANbitRate)
{
    ...
    {
        log_e("Verify arguments! (Nullpointer in CANmodule|rxArray|txArray)");
    ...
    case 1000:
        ...
        break;
    default:
        log_e("%d => Invalid Baudrate! Using 1Mbit as default!", CANbitRate);
        //Set Baudrate to 1Mbit;
        timingConfig.brp = 4;
        timingConfig.tseg_1 = 15;
        timingConfig.tseg_2 = 4;
    ...
    if (CANmodule->useCANrxFilters)
    {
        ...
        log_i("RxFilters are active, but no filter configured.");
    ...
    void (*CANrx_callback)(void *object, void *message))
{
    ...
    if ((CANmodule != NULL) && (object != NULL) && (CANrx_callback != NULL) && (index < CANmodule->rxSize))
    {
        ...
        if (CANmodule->useCANrxFilters)
        {
            log_e("No driver implementation for Filters");
        }
    }
    else
    {
        log_e("((CANmodule!=NULL) && (object!=NULL) && (pFunct!=NULL) && (index < CANmodule->rxSize))==FALSE");
    ...
    bool_t syncFlag)
{
    ...
    {
        log_e("CANmodule not initialized or index out of bound of txSize");
    }
    return buffer;
}
...
CO_ReturnError_t CO_CANsend(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer)
{
    ...
    if (buffer->bufferFull)
    {
        ...
        log_e("CAN-tx-buffer overflow 1");
    ...
    if ((esp_can_hw_status.msgs_to_tx < CAN_TX_QUEUE_LENGTH) && (CANmodule->CANtxCount == 0))
    {
        ...
        if (send_err == ESP_OK)
        {
            ...
            // log_i("ID: %d , Data %d,%d,%d,%d,%d,%d", temp_can_message.identifier, temp_can_message.data[0], temp_can_message.data[1], temp_can_message.data[2], temp_can_message.data[3], temp_can_message.data[4], temp_can_message.data[5]);
        }
        else
        {
            err = CO_ERROR_TIMEOUT;
            log_e("Failed to queue message for transmission - %u", send_err);
        }
    }
    /* if no buffer is free, message will be sent by interrupt */
    else
    {
        log_e("CAN-tx-buffer overflow 2 - %u - %u", esp_can_hw_status.msgs_to_tx, CANmodule->CANtxCount);
...
void CO_CANclearPendingSyncPDOs(CO_CANmodule_t *CANmodule)
{
    ...
    log_e("This function is NOT implemented!");
