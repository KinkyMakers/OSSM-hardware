...
void getUserInputTask(void *pvParameters) {
    ...
    for (;;)  // tasks should loop forever and not return - or will throw error
              // in OS
    {
        ossm.updateAnalogInputs();
        ossm.handleStopCondition();

        if (digitalRead(WIFI_CONTROL_TOGGLE_PIN) ==
            HIGH)  // TODO: check if wifi available and handle gracefully
        {
            ossm.enableWifiControl();
        } else {
            if (ossm.wifiControlActive == true) {
                // this is a transition to local control, we should tell the
                // server it cannot control
        ...
        if (ossm.speedPercentage > ossm.commandDeadzonePercentage) {
            targetSpeedMmPerSecond =
                ossm.maxSpeedMmPerSecond * ossm.speedPercentage / 100.0;
            if ((ossm.speedPercentage - previousSpeedPercentage) > 30) {
                targetSpeedMmPerSecond =
                    0.5 * (ossm.speedPercentage + previousSpeedPercentage) *
                    ossm.maxSpeedMmPerSecond / 100.0;
            }
            targetAccelerationMm = ossm.maxSpeedMmPerSecond *
                                   ossm.speedPercentage * ossm.speedPercentage /
                                   ossm.accelerationScaling;

            ossm.stepper.setAccelerationInMillimetersPerSecondPerSecond(
                targetAccelerationMm);
            if (targetSpeedMmPerSecond >
                ossm.stepper
                    .getCurrentVelocityInMillimetersPerSecond()) {  // we are
                                                                    // speeding
                                                                    // up, we
                                                                    // need to
                                                                    // increase
            ...
            // If target speed is lower than current, we do not update deccel as
            // setting a low decel when going from high to low speed causes the
            // motor to travel a long distance before slowing.
        }
