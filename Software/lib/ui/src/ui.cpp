...
void setFooter(u8g2_t* u8g2, const char* left, const char* right) {
    ...
    u8g2_DrawStr(u8g2,
                 SCREEN_WIDTH - u8g2_GetStrWidth(u8g2, rightUpper.c_str()) - 8,
...
void drawTextPage(u8g2_t* u8g2, const TextPage& page) {
    ...
    int textW = qrPixelWidth > 0 ? SCREEN_WIDTH - qrPixelWidth - 4
                                  : SCREEN_WIDTH;
    int y = 0;
...
void drawPlayControls(u8g2_t* u8g2, const PlayControlsData& data) {
    ...
    const char* strokeLabel = data.strokeLabel ? data.strokeLabel : strings::stroke;
    ...
    if (data.isStrokeEngine) {
        switch (data.activeControl) {
            ...
            case PlayControl::SENSATION:
                drawShape::settingBar(u8g2, strings::sensation, data.sensation, 128, 0,
                                      RIGHT_ALIGNED, 10);
            ...
            case PlayControl::DEPTH:
                drawShape::settingBarSmall(u8g2, data.sensation, 125);
                drawShape::settingBar(u8g2, strings::depth, data.depth, 123, 0,
                                      RIGHT_ALIGNED, 5);
                drawShape::settingBarSmall(u8g2, data.stroke, 108);
                break;
            default:
        ...
        switch (data.activeControl) {
            case PlayControl::BUFFER:
                drawShape::settingBar(u8g2, strings::buffer, data.buffer, 128, 0,
                                      RIGHT_ALIGNED, 15);
            ...
            case PlayControl::STROKE:
                ...
                drawShape::settingBar(u8g2, strokeLabel, data.stroke, 113, 0,
                                      RIGHT_ALIGNED);
                break;
            case PlayControl::SENSATION: {
                drawShape::settingBarSmall(u8g2, data.buffer, 125);
                drawShape::settingBar(u8g2, strings::max, data.sensation, 123, 0,
                                      RIGHT_ALIGNED, 10);
                u8g2_SetFont(u8g2, Font::bold);
            ...
            case PlayControl::DEPTH:
                drawShape::settingBarSmall(u8g2, data.buffer, 125);
                drawShape::settingBarSmall(u8g2, data.sensation, 120);
                drawShape::settingBar(u8g2, strings::depth, data.depth, 118, 0,
                                      RIGHT_ALIGNED, 5);
                drawShape::settingBarSmall(u8g2, data.stroke, 103);
                break;
        }
...
void drawMenu(u8g2_t* u8g2, const MenuData& data) {
    ...
    if (n > 1) {
        ...
        u8g2_DrawUTF8(u8g2, leftPadding, itemHeight * 1, data.items[prevIdx]);

        int nextIdx = (idx + 1) % n;
        u8g2_DrawUTF8(u8g2, leftPadding, itemHeight * 3, data.items[nextIdx]);
    }
...
void drawHeaderIcons(u8g2_t* u8g2, const HeaderIconsData& data) {
    ...
    switch (data.wifi) {
        ...
        case WifiStatus::CONNECTING:
            u8g2_DrawGlyph(u8g2, WIFI_ICON_X, ICON_Y, IconGlyph::WIFI_CONNECTING);
            break;
        case WifiStatus::CONNECTED:
            u8g2_DrawGlyph(u8g2, WIFI_ICON_X, ICON_Y, IconGlyph::WIFI_CONNECTED);
            break;
    ...
    switch (data.ble) {
        case BleStatus::CONNECTED:
            u8g2_DrawGlyph(u8g2, BLE_ICON_X, ICON_Y, IconGlyph::BLE_CONNECTED);
            break;
        case BleStatus::DISCONNECTED: {
            constexpr int cx = BLE_ICON_X + 4;
            constexpr int cy = ICON_Y - 4;
            u8g2_DrawCircle(u8g2, cx, cy, 3, U8G2_DRAW_ALL);
            break;
        }
        case BleStatus::CONNECTING:
        case BleStatus::ADVERTISING:
            u8g2_DrawGlyph(u8g2, BLE_SUB_X, ICON_Y, IconGlyph::BLE_SMALL);
            u8g2_DrawPixel(u8g2, BLE_IND_X, ICON_Y);
            u8g2_DrawPixel(u8g2, BLE_IND_X + 2, ICON_Y);
            u8g2_DrawPixel(u8g2, BLE_IND_X + 4, ICON_Y);
            break;
